...
import type { PreflopData, PlayerAction } from "@/types/post"
import { CardSelector } from "@/components/card-selector"

interface Props {
  data: PreflopData
  onUpdate: (data: Partial<PreflopData>) => void
}

export function PostCreationStep2({ data, onUpdate }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-6">
        <h3 className="text-xl font-bold text-foreground mb-2">プリフロップ</h3>
        <p className="text-muted-foreground">ハンドとプリフロップでのアクションを入力してください</p>
      </div>
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <div className="flex gap-4">
            <CardSelector
              value={data.holeCards?.[0] || null}
              onChange={(card) => onUpdate({ holeCards: [card, data.holeCards?.[1] || null] })}
              placeholder="1枚目"
            />
            <CardSelector
              value={data.holeCards?.[1] || null}
              onChange={(card) => onUpdate({ holeCards: [data.holeCards?.[0] || null, card] })}
              placeholder="2枚目"
            />
          </div>
        </div>
        ...
        <div className="space-y-2">
          <Label htmlFor="description">プリフロップの詳細</Label>
          <Textarea
            id="description"
            placeholder="プリフロップでの思考過程や相手の反応など"
